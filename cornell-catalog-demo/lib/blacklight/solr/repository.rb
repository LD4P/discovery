  ...
  class Repository < Blacklight::AbstractRepository
    ...
    def send_and_receive(path, solr_params = {})
      benchmark("Solr fetch", level: :debug) do
        Rails.logger.info("REPOSITORY: PATH = " + path.inspect)
        key = blacklight_config.http_method == :post ? :data : :params
        Rails.logger.info("REPOSITORY: KEY = " + key.inspect)
        res = connection.send_and_receive(path, {key=>solr_params.to_hash, method: blacklight_config.http_method})
        Rails.logger.info("REPOSITORY: ARE WE THERE YET")
        ...
        Rails.logger.info("Solr response: #{solr_response.inspect}")
        solr_response
      end
