var processNarrativeLocation = {
  onLoad: function() {
      ...
      if ( workId.length ) {
          processNarrativeLocation.getWikiLocalName(workId);
      }
  },
    ...
    $.ajax({
      ...
      success : function(data) {
        if ( data['results']['bindings'].length ) {
            ...
            processNarrativeLocation.getWikiDerivatives(URI);
            processNarrativeLocation.getWikiEditions(URI);
            processNarrativeLocation.getNarrativeLocations(URI);
        } 
        else {
      ...
      success : function (data) {
        if (data && "results" in data
          ...
          if (bindings.length) {
            ...
            $("#itemDetails").append(processNarrativeLocation.generateItemViewRow(fieldName, fieldValue.join("<br/>")));
...
Blacklight.onLoad(function() {
  if ( $('body').prop('className').indexOf("catalog-show") >= 0 ) {
    processNarrativeLocation.onLoad();
  }
});  
